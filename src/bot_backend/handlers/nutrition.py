...
from bot_backend.keyboards import (
    ...
    get_days_keyboard,  # ✅ новая клавиатура для выбора дней
    MAIN_MENU_BUTTON
)
...
def _get_nutrition_response(user_id: int) -> tuple:
    ...
    return plan_text, ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
...
async def handle_budget(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.debug("handle_budget запущен")
    user_id = update.effective_user.id
    user_data = db.get_user(user_id)
