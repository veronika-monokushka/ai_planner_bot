...
from states import UserState, UserData
from keyboards import *
from database import db, calculate_bmi, calculate_calories, get_bmi_category, recalculate_user_data, generate_meal_plan, generate_shopping_list, get_motivational_message
...
async def handle_shopping_list_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📝 Собрать список на неделю":
        ...
        if not user_recipes:
            shopping_list = generate_shopping_list(active_plan.get('plan', {}))
        else:
            shopping_list = generate_shopping_list(active_plan.get('plan', {}))
